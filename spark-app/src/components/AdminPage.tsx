...
export default function AdminPage({ onClose }: Props) {
  const [tab, setTab] = useState<"feedback" | "evals">("feedback");
  const [liveFeedback, setLiveFeedback] = useState<FeedbackItem[]>([]);
  const [selectedFb, setSelectedFb] = useState<string>("fb1");
  ...
  useEffect(() => {
    void fetch("/api/feedback")
      .then((r) => r.json())
      .then((data: { feedback: { topicId: string; rating: string; reasons?: string[]; note?: string; handle?: string }[] }) => {
        const mapped: FeedbackItem[] = (data.feedback || []).map((f, i) => ({
          id: `live-${i}`,
          when: "just now",
          user: "you",
          target: f.handle?.replace(/.*\/in\//, "") || "unknown",
          rating: (f.rating === "up" ? "up" : "down") as "up" | "down",
          reasons: f.reasons || [],
          note: f.note || "",
          starter: `Topic ${f.topicId}`,
          why: "Live feedback from this session",
        }));
        setLiveFeedback(mapped);
      })
      .catch(() => {});
  }, []);

  const allFeedback = useMemo(() => [...liveFeedback, ...SEED_FEEDBACK], [liveFeedback]);
  const fb = useMemo(() => allFeedback.find((f) => f.id === selectedFb), [selectedFb, allFeedback]);
  const downvotes = allFeedback.filter((f) => f.rating === "down");
    ...
    <div className="page wide fade-in" data-screen-label="07 admin">
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <span
            className="badge"
            style={{ background: "#1a1a17", borderColor: "#1a1a17", color: "#fbf5e6" }}
          >
            SSO · ENG ONLY
          </span>
      ...
      >
        {(
          [
            ["feedback", "Feedback queue", downvotes.length],
            ["evals", "Eval sets", EVAL_SETS.length],
          ] as const
        ).map(([k, label, count]) => (
          <button
            key={k}
            ...
            {downvotes.map((f) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  borderLeft:
                    selectedFb === f.id ? "2px solid var(--ink)" : "2px solid transparent",
                }}
              >
                <div
                  style={{
                ...
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                  {f.reasons.map((r) => (
                    <span
                      key={r}
                      className="chip"
                      style={{ height: 18, fontSize: 10, padding: "0 7px", cursor: "default" }}
                    >
                      {r}
                    </span>
                  ))}
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                ...
                <span className="muted" style={{ fontSize: 12 }}>
                  about{" "}
                  <strong style={{ color: "var(--ink-2)" }}>{fb.target}</strong>
                </span>
              </div>
              ...
              {fb.note && (
                ...
                >
                  <div className="eyebrow" style={{ color: "#a14a44", marginBottom: 4 }}>
                    USER NOTE
                  </div>
                  &ldquo;{fb.note}&rdquo;
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  ...
                  <div className="card" style={{ padding: 16, opacity: 0.9 }}>
                    <p className="topic-pull" style={{ fontSize: 16, marginBottom: 10 }}>
                      {fb.starter}
                    </p>
              ...
              >
                <div className="muted" style={{ fontSize: 12 }}>
                  Save as eval row · adds to{" "}
                  <strong style={{ color: "var(--ink-2)" }}>Generic-output regression</strong>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setSavedAs(
                        `Saved to "Generic-output regression" · row #${
                          15 + Math.floor(Math.random() * 5)
                        }`
                      )
                    }
                  >
                    <Icon name="check" /> Save as eval
                  </button>
                </div>
              </div>
              {savedAs && (
          ...
          {EVAL_SETS.map((s, i) => (
            ...
            >
              <div>
                ...
                <div className="muted" style={{ fontSize: 12 }}>
                  {s.count} rows · last run {s.lastRun}
                </div>
              </div>
